...
- **Toetsenbord & undo aan de deur**: in het Check-in Station navigeer je met `↑`/`↓` door de lijst (zichtbare focus-ring), checkt `Enter` de gemarkeerde gast in (groepen openen het paneel), springt `/` naar het zoekveld en wist `Esc`. Elke (gedeeltelijke) check-in en checkout toont 8 s een toast met "Ongedaan maken" die de volledige vóór-status herstelt. Zoeken dekt ook tafelnummer en aangepaste-veldwaarden; de ingecheckt/totaal-teller staat op elk schermformaat in de header. De Tekst-tab van het gasten-toevoegpaneel ondersteunt de volledige plak-syntax (`+X`, `(gratis)`/`(N)`, `: notitie`, e-mail in de regel) met live parse-preview en rood gemarkeerde foutregels. Zie `rapport-attendium-gap-analyse.md`.
- **Camera-QR-scanner**: het QR-tabblad van het Check-in Station (`src/components/checkin/QRScanCheckIn.jsx`) scant nu echt met de camera: `getUserMedia` (achtercamera) + `jsqr` decodeert live het videobeeld (elke 150 ms op een verkleind frame, dubbele scans binnen 3 s genegeerd). Bij herkenning pauzeert de scanner en verschijnt de gastkaart ter bevestiging (check-in/uitchecken); daarna scant hij verder. Handmatige invoer blijft als terugvaloptie (ook voor USB-scanners die als toetsenbord werken), met nette meldingen bij geweigerde cameratoegang of ontbrekende camera. Teksten van het tabblad zijn meteen vernederlandst.
